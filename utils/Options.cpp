...
namespace Options {
    ...
    for (int idx = start; idx < data.size(); idx++) {
      if (data[idx] == 0) {
        if (inKey) {
          ...
          options[numOptions++] = std::make_pair(key, value);
          key.clear();
          value.clear();
          inKey = true;
          continue;
  ...
  map_t create(long blksize, long timeout, long tsize) {
    ...
    options[0] = std::make_pair("blksize", blksize);
    options[1] = std::make_pair("timeout", timeout);
    options[2] = std::make_pair("tsize", tsize);
  ...
  map_t validate(const map_t &options) {
    ...
    for (const auto &[order, item]: options) {
      ...
      if (key == "blksize") {
        ...
        validated[0] = std::make_pair("blksize", result);
      } else if (key == "timeout") {
        try {
        ...
        validated[1] = std::make_pair("timeout", result);
      } else if (key == "tsize") {
        try {
        ...
        validated[2] = std::make_pair("tsize", result);
      }
    }
    return validated;
  }
